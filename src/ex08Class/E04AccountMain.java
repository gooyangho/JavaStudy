...
class Account {
	//멤버변수 
	String name; //계좌 주인의 이름   String : 문자열
	...
	void showAccount() { //계좌 정보를 출력하는 메서드
		System.out.println("계좌주:" + name);
		System.out.println("계좌번호:"+ accountNumber);
		System.out.println("잔고:" + balance);
	}
	//멤버변수를 초기화 할 목적으로 생성된 멤버메서드
	void init(String n, String a, int b) { //계좌 객체를 초기화하는 메서드
		name = n;
		accountNumber = a;
		balance =b;
	}
...
public class E04AccountMain {
	...
	public static void main(String[] args) {
		...
		account.init("장동건", "111-21-8888", 1000); // 계좌 초기화
		//입/출금을 진행한 후 계좌정보 확인
		account.deposit(9000); //입금
		account.withdraw(5000); //출금
		account.showAccount(); //계좌 정보 출력
		
		//두번째계좌
		Account account2 =new Account();
		//멤버변수에 직접 접근해서 인스턴스를 초기화
		account2.name = "정우성"; //계좌주 이름설정
		account2.accountNumber = "123-45-67890"; //계좌 번호설정
		account2.balance = 900000; // 잔액 설정
		//잔고가 부족한 경우 출금되지 않음
		account2.withdraw(990000);
		account2.showAccount(); // 계좌 정보 출력
	}
}
